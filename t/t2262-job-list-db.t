...
test_expect_success HAVE_JQ 'flux job list-inactive jobs w/ since 1 (pre purge)' '
...
test_expect_success HAVE_JQ 'flux job list-inactive jobs w/ since 2 (pre purge)' '
        count=$(job_list_state_count inactive) &&
        count=$((count - 1)) &&
...
test_expect_success HAVE_JQ 'flux job list-ids jobs (pre purge)' '
...
test_expect_success HAVE_JQ 'flux job list-inactive jobs w/ since 1 (post purge)' '
        timestamp=`flux job list -s inactive | head -n 2 | tail -n 1 | jq .t_inactive` &&
...
test_expect_success HAVE_JQ 'flux job list-inactive jobs w/ since 2 (post purge)' '
        count=$(cat inactive.ids.orig | wc -l) &&
        count=$((count - 1)) &&
        timestamp=`flux job list -s inactive | tail -n 1 | jq .t_inactive` &&
...
test_expect_success HAVE_JQ 'flux job list-ids jobs (post purge)' '
...
test_expect_success HAVE_JQ 'flux job list-inactive jobs w/ since 1 (all purge)' '
        timestamp=`flux job list -s inactive | head -n 2 | tail -n 1 | jq .t_inactive` &&
...
test_expect_success HAVE_JQ 'flux job list-inactive jobs w/ since 2 (all purge)' '
        count=$(cat inactive.ids.orig | wc -l) &&
        count=$((count - 1)) &&
        timestamp=`flux job list -s inactive | tail -n 1 | jq .t_inactive` &&
...
test_expect_success HAVE_JQ 'flux job list-ids jobs (all purge)' '
        for id in `cat inactive.ids.orig`
        do
            flux job list-ids ${id} | jq -e ".id == ${id}"
        done
