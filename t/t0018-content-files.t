...
# Usage: checkpoint_backing_put key rootref
checkpoint_backing_put() {
        o="{key:\"$1\",value:{version:1,rootref:\"$2\",timestamp:2.2}}"
        jq -j -c -n  ${o} | $RPC content-backing.checkpoint-put
}
# Usage: checkpoint_backing_get key >value
...
test_expect_success 'load content-files module on rank 0 and content flush' '
...
wait_checkpoint_flush() {
        ...
        while checkpoint_backing_get foo \
              ...
              && [ $i -lt 50 ]
        do
            cat checkpointflush.out
            if [ "$(cat checkpointflush.out)" == "$expected" ]
            then
                return 0
            fi
            sleep 0.1
