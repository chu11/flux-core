...
test_expect_success HAVE_JQ 'flux job list running jobs with correct states_mask' '
        ...
        flux job list -s run | jq .states_mask | ${JOB_CONV} statesmasktostr > list_states_mask_R.out &&
...
test_expect_success HAVE_JQ 'flux job list inactive jobs with correct states_mask' '
        echo "NEW,DEPEND,PRIORITY,SCHED,CLEANUP,INACTIVE" >> list_states_mask_I.exp
        for count in `seq 1 5`; do \
            echo "NEW,DEPEND,PRIORITY,SCHED,RUN,CLEANUP,INACTIVE" >> list_states_mask_I.exp; \
        done &&
        flux job list -s inactive | jq .states_mask | ${JOB_CONV} statesmasktostr > list_states_mask_I.out &&
...
test_expect_success HAVE_JQ 'flux job list pending jobs with correct states_mask' '
        for count in `seq 1 8`; do \
            echo "NEW,DEPEND,PRIORITY,SCHED" >> list_states_mask_S.exp; \
        done &&
        flux job list -s sched | jq .states_mask | ${JOB_CONV} statesmasktostr > list_states_mask_S.out &&
