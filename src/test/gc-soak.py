...
def main():
    ...
    # are genuinely subject to sweep, not merely cache-resident.  A
    # FLUX_KVS_SYNC commit flushes dirty cache entries and checkpoints in one
    # step, so no separate content flush is needed.
    flux.kvs.commit(handle, flags=flux.constants.FLUX_KVS_SYNC)
