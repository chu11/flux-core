...
static int overlay_sendmsg_parent (struct overlay *ov, const flux_msg_t *msg)
{
    ...
    rc = zsock_msg_sendzsock (ov->parent.zsock, msg);
    if (rc == 0)
...
static int overlay_sendmsg_child (struct overlay *ov, const flux_msg_t *msg)
{
    ...
    rc = zsock_msg_sendzsock_ex (ov->bind_zsock, msg, true);
done:
    return rc;
}
                      ...
                      int revents, void *arg)
{
    ...
    if (!(msg = zsock_msg_recvzsock (ov->bind_zsock)))
        return;
    if (flux_msg_get_type (msg, &type) < 0
                       ...
                       int revents, void *arg)
{
    ...
    if (!(msg = zsock_msg_recvzsock (ov->parent.zsock)))
        return;
    if (flux_msg_get_type (msg, &type) < 0) {
        goto drop;
    }
