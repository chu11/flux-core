...
static json_t *drain_get_state (struct drain *drain)
{
    json_t *o = NULL;
    const char *key;
    json_t *value;
...
void drain_checkpoint (struct drain *drain)
{
    ...
    if (!(drain_state = drain_get_state (drain))
...
struct drain *drain_create (struct resource_ctx *ctx, const json_t *eventlog)
{
    struct drain *drain;
    flux_error_t error;
    double checkpoint_timestamp = 0.0;
    ...
    if (!(drain->info = calloc (drain->size, sizeof (drain->info[0]))))
        goto error;
    if (load_drain_state (drain, &checkpoint_timestamp, &error) < 0) {
        flux_log (ctx->h, LOG_ERR, "%s: %s", DRAIN_STATE_CHECKPOINT, error.text);
        goto error;
    }
    if (replay_eventlog (drain, checkpoint_timestamp, eventlog, &error) < 0) {
