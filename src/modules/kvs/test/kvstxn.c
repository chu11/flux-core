...
void kvstxn_process_refresh_test (void)
{
    ...
    /* Flush the root cache entry to make it valid+CLEAN, emulating what the
     * KVS does after storing it to the content backing store.  Look it up
     * directly and clear its dirty bit (it was never on a dirty list we could
     * iterate here, since the no-op transaction 2 went straight to FINISHED).
     */
    {
        struct cache_entry *entry = cache_lookup (cache, rootref);
        ok (entry != NULL && cache_entry_get_dirty (entry),
            "root cache entry is present and dirty before flush");
        cache_entry_set_dirty (entry, false);
    }

    /* Transaction 3: write the SAME value to the SAME key once more.  Now the
     * root cache entry is valid+CLEAN (flushed above), so store_cache() returns
     * STORE_CACHE_REFRESH rather than NOOP: the entry is scheduled for a
     * best-effort re-store to refresh its GC epoch, without being marked dirty.
     * This exercises the refresh_count > 0 path.
     */
    create_ready_kvstxn (ktm, "transaction3", "testkey", "testvalue", 0, 0);

    ok ((kt = kvstxn_mgr_get_ready_transaction (ktm)) != NULL,
        "kvstxn_mgr_get_ready_transaction returns ready kvstxn (refresh commit)");

    ok (kvstxn_process (kt, rootref, 0) == KVSTXN_PROCESS_FINISHED,
        "kvstxn_process returns KVSTXN_PROCESS_FINISHED (refresh commit)");

    ok ((newroot = kvstxn_get_newroot_ref (kt)) != NULL,
        "kvstxn_get_newroot_ref returns != NULL when processing complete (refresh commit)");

    /* The valid+clean root took the REFRESH path, so it lands on the refresh
     * list.  kvstxn_process() returned FINISHED (not DIRTY_CACHE_ENTRIES),
     * which already confirms nothing was marked dirty -- a refresh must never
     * mark a live blob dirty.
     */
    refresh_count = 0;
    ok (kvstxn_iter_refresh_cache_entries (kt, cache_count_refresh_cb, &refresh_count) == 0,
        "kvstxn_iter_refresh_cache_entries works (refresh commit)");

    ok (refresh_count == 1,
        "one refresh cache entry for valid+clean no-op commit");

    ok (streq (newroot, rootref),
        "newroot equals oldroot for refresh commit");
