...
int job_db_store (struct job_db_ctx *ctx, struct job *job)
{
    ...
    if (!(eventlog_str = json_dumps (job->eventlog, JSON_COMPACT))) {
        flux_log_error (ctx->h, "json_dumps eventlog");
        goto out;
    }
    if (sqlite3_bind_text (ctx->store_stmt,
                           15,
    ...
    if (!(jobspec = json_dumps (job->jobspec, JSON_COMPACT))) {
    ...
    if (job->R) {
        if (!(R = json_dumps (job->R, JSON_COMPACT))) {
