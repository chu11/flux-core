...
struct flux_msg {
    ...
    /* to avoid unnecessary mem allocations */
                         ...
                         int *iovcntp)
{
    ...
    index = frame_count - 1;
    /* we cast away const here.  msg->proto is only used
     * internally to avoid an extra allocation and is not
     * accessed publicly
     */
    msg_proto_setup (msg, (uint8_t *)msg->proto, PROTO_SIZE);
    iov[index].data = msg->proto;
