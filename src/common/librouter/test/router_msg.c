...
void check_sendzsock (void)
{
    ...
    ok (router_msg_sendzsock (zsock[1], any) < 0 && errno == EPROTO,
        "router_msg_sendzsock returns < 0 and EPROTO on msg w/ type = ANY");
    ok (router_msg_sendzsock_ex (NULL, msg, true) < 0 && errno == EINVAL,
        "router_msg_sendzsock_ex returns < 0 and EINVAL on dest = NULL");
    ok (router_msg_sendzsock_ex (zsock[1], any, true) < 0 && errno == EPROTO,
        "router_msg_sendzsock_ex returns < 0 and EPROTO on msg w/ type = ANY");
